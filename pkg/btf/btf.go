...
func ResolveBTFPath(
	...
	case *btf.Struct:
		return processMembers(btfArgs, btfPtrName, currentType, t.Members, pathToFound, i)
	case *btf.Union:
		return processMembers(btfArgs, btfPtrName, currentType, t.Members, pathToFound, i)
	case *btf.Pointer:
		if len(pathToFound[i]) == 0 {
			...
			(*btfPtrName)[i] = GetPtrName(pathToFound[:i])
			return ResolveBTFPath(btfArgs, btfPtrName, ResolveNestedTypes(t.Target), pathToFound, i+1)
		}
		(*btfArgs)[i-1].IsPointer = uint16(1)
		(*btfPtrName)[i-1] = GetPtrName(pathToFound[:i])
		return ResolveBTFPath(btfArgs, btfPtrName, ResolveNestedTypes(t.Target), pathToFound, i)
	default:
...
func processMembers(
	...
	for _, member := range members {
		if len(member.Name) == 0 { // If anonymous struct, fallthrough
			...
			(*btfArgs)[i].IsInitialized = uint16(1)
			lastTy, err := ResolveBTFPath(btfArgs, btfPtrName, ResolveNestedTypes(member.Type), pathToFound, i)
			if err != nil {
				if lastError != nil {
					idx := i + 1
		...
		if member.Name == pathToFound[i] {
			...
			if isNotLastChild {
				return ResolveBTFPath(btfArgs, btfPtrName, ResolveNestedTypes(member.Type), pathToFound, i+1)
			}
